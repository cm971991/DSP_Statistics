...
//验证概率0<obj<=1,或者null,空
function isChance(obj) {
    var controlObj = $.trim(obj);
    if (controlObj.length == 0 || controlObj == null || controlObj == undefined) {
        return false;
    }
    ...
    $(obj).find("[datacol=yes]").each(function () {
        if ($(this).attr("checkexpession") != undefined) {
            ...
            switch ($(this).attr("checkexpession")) {
                ...
                case "NumOrNull":
                    {
                        if (!isIntegerOrNull(value)) {
                            Validatemsg = $(this).attr("err") + "必须为>0数字！\n";
                            Validateflag = false;
                            tipCss($(this), Validatemsg); return false;
                        }
                        break;
                ...
                case "NoChineseStrOrNull":
                    {
                        if (isChineseOrNull(value, $(this).attr("eqvalue"))) {
                            Validatemsg = $(this).attr("err") + "不能包含为中文！\n";
                            Validateflag = false;
                            tipCss($(this), Validatemsg); return false;
                        }
                        break;
                ...
                case "IsChance":
                    if (!isChance(value, $(this).attr("eqvalue"))) {
                        Validatemsg = $(this).attr("err") + "必须为大于0小于或等于1！\n";
                        Validateflag = false;
                        tipCss($(this), Validatemsg); return false;
                    }
                    break;
                case "Manager":
                    if (!IsManager(value)) {
                        Validatemsg = $(this).attr("err") + "只能为6-20个字母、数字、下划线！\n";
                        Validateflag = false;
                        tipCss($(this), Validatemsg); return false;
                    }
                default:
